...
def _payload(row):
    try: return json.loads((row["payload_json"] if "payload_json" in row.keys() else None) or "{}")
    except (TypeError,json.JSONDecodeError): return {}
def _haystack(row,payload): return " ".join(str(x or "") for x in (row["title"],row["agency"],row["stage"],json.dumps(payload,ensure_ascii=False))).lower()
...
def _deadline(payload):
    ...
    for key in ("ResponseDeadLine","response_deadline","CloseDate","closedate","close_date"):
        parsed=_date(row.get(key) if isinstance(row,dict) else None)
...
def score_event(row,identifiers,profile,as_of):
    payload=_payload(row); haystack=_haystack(row,payload); event_date=_date(row["event_date"]); lookback=int(profile.get("lookback_days",7))
    if not event_date or (as_of-event_date).days < 0 or (as_of-event_date).days > lookback: return None
    score=max(0,28-(as_of-event_date).days*4); reasons=[]
    keyword_hits=[x for x in profile.get("keywords",[]) if x.lower() in haystack]
    if keyword_hits: score+=min(26,8+len(set(keyword_hits))*3); reasons.append("topic: "+", ".join(sorted(set(keyword_hits))[:4]))
    geo_hits=[x for x in profile.get("geographies",[]) if x.lower() in haystack]
    if geo_hits: score+=24; reasons.append("geography: "+", ".join(sorted(set(geo_hits))[:3]))
    naics=set(identifiers.get("naics",[])); hits=sorted(naics & set(str(x) for x in profile.get("naics",[])))
    if hits: score+=26; reasons.append("NAICS: "+", ".join(hits[:3]))
    agency=str(row["agency"] or ""); agency_hits=[x for x in profile.get("agencies",[]) if x.lower() in agency.lower()]
    if agency_hits: score+=9; reasons.append("agency: "+agency_hits[0])
    amount=float(row["amount"]) if row["amount"] is not None else None
    if amount is not None:
        if abs(amount)>=float(profile.get("high_value_amount",500000)): score+=10; reasons.append(f"value: ${abs(amount):,.0f}")
        elif abs(amount)>=float(profile.get("minimum_amount",25000)): score+=5; reasons.append(f"value: ${abs(amount):,.0f}")
    deadline=_deadline(payload); days=(deadline-as_of).days if deadline else None
    if days is not None and 0<=days<=int(profile.get("closing_soon_days",10)): score+=12; reasons.append(f"closes in {days} days")
    kind=str(row["kind"] or "")
    if kind in {"contract_opportunity","funding_opportunity"}: score+=8
    elif kind=="federal_award_action" and amount: score+=4
    if not (keyword_hits or geo_hits or hits or agency_hits): return None
    return {"event_id":row["event_id"],"source":row["source"],"kind":kind,"lane":lane_for(kind,days),"stage":row["stage"],"title":row["title"],"agency":row["agency"],"event_date":row["event_date"],"amount":amount,"currency":row["currency"],"official_url":row["official_url"],"score":round(score,1),"reasons":reasons,"days_to_close":days,"identifiers":identifiers}
def rank_opportunities(conn,as_of,profile_name="default",limit=30):
    ...
    ranked.sort(key=lambda x:(-x["score"],x.get("days_to_close") if x.get("days_to_close") is not None else 9999,x["event_id"])); return ranked[:limit]
def rank_all_profiles(conn,as_of,limit=30):
    profiles=load_profiles(); by_profile={name:rank_opportunities(conn,as_of,name,limit) for name in profiles}
    combined={}
    ...
    items=list(combined.values()); items.sort(key=lambda x:-x["score"])
    lanes={key:[x for x in items if x["lane"]==key] for key in ("act_now","market_intelligence","policy_signals")}
    return by_profile,lanes,items[:limit]
def publish_opportunities(conn,as_of,out_dir,now,*,profile_name="default",freshness=None):
    ...
    by_profile,lanes,items=rank_all_profiles(conn,as_of); profiles=load_profiles()
