...
from .taxonomy import AgencyIdentity, canonicalize_agency, taxonomy_versions, watchlist_matches
...
def detect_standalone(conn, as_of: str, lookback_days: int = 14) -> list[dict]:
    ...
    for raw in rows:
        ...
        out.append({"record_id":row["id"],"signal_key":f"standalone:{row['id']}","canonical_agency_id":identity.canonical_id,"canonical_agency_name":identity.canonical_name,"raw_agency_name":row.get("agency"),"title":row.get("title"),"doc_type":row.get("doc_type"),"publication_date":row.get("publication_date"),"official_url":row.get("url"),"matches":matches,"taxonomy_versions":taxonomy_versions(),"evidence":{"topics":json.loads(row.get("subjects") or "[]") if isinstance(row.get("subjects"),str) else row.get("subjects") or [],"title":row.get("title"),"doc_type":row.get("doc_type"),"taxonomy_versions":taxonomy_versions()}})
    return out
