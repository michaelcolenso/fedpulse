...
def run_pipeline(db_path: Path, out_dir: Path, as_of: str | None = None, *, ingest_fr: bool = True, sync_marc: bool = True, fr_fetcher: Callable | None = None, marc_syncer: Callable | None = None) -> int:
    as_of=as_of or dt.datetime.now(dt.timezone.utc).date().isoformat(); now=dt.datetime.now(dt.timezone.utc); out_dir=Path(out_dir)
    with acquire_lock(out_dir/".pipeline.lock"):
        ...
        if sync_marc:
            ...
            try:
                (marc_syncer or marc_sync.sync)(); record_success(conn,"marc",stamp,"maintenance sync complete")
            except Exception as exc:
