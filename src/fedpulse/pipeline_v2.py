    ...
    with acquire_lock(lock_path):
        ...
        if sync_keyless:
            ...
            try:
                ...
                detail = "; ".join(f"{name}={result.get('events', result.get('edges_created', result.get('status')))}" for name,result in results.items())
                if failed:
                    record_failure(conn,"keyless_sources",stamp,f"partial failure: {','.join(failed)}; {detail}")
                else:
                    record_success(conn,"keyless_sources",stamp,detail)
            except Exception as exc:
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--db",type=Path,default=Path("data/fedpulse.db")); parser.add_argument("--out",type=Path,default=Path("data/outputs")); parser.add_argument("--as-of")
    parser.add_argument("--skip-ingest",action="store_true"); parser.add_argument("--skip-marc",action="store_true")
    parser.add_argument("--skip-keyless-sources",action="store_true",help="Skip v0.4 zero-key sources")
    parser.add_argument("--skip-bulk-sources",action="store_true",help="Skip large Grants.gov and SAM.gov bulk downloads")
