...
class TestAgencies(unittest.TestCase):
    ...
    def test_mapping_version_skips_unchanged_rows_and_reprocesses_on_version_change(self):
        row = load_case("cdc_funding_package")[0]
        with temp_db() as conn:
            seed_records(conn, [row])
            first = normalize_all(conn)
            second = normalize_all(conn)
            self.assertEqual(first["mapped"], second["mapped"])
            self.assertEqual(conn.execute("select count(*) from records where agency_mapping_version is not null").fetchone()[0], 1)
            from fedpulse import taxonomy
            old = taxonomy.AGENCY_CONFIG["version"]
            try:
                taxonomy.AGENCY_CONFIG["version"] = "agency-v-test"
                changed = normalize_all(conn)
                self.assertEqual(changed["processed"], 1)
                self.assertEqual(conn.execute("select agency_mapping_version from records").fetchone()[0], "agency-v-test")
            finally:
                taxonomy.AGENCY_CONFIG["version"] = old
